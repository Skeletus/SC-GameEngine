...
namespace sc
{
  ...
  struct WorldPartitionFrameStats
  {
    ...
    uint32_t activeRadiusSectors = 0;
  ...
  public:
    ...
    void updateActiveSet(const Vec3& cameraPos, uint32_t radiusInSectors, const WorldPartitionBudget& budget);
    ...
    uint32_t loadedSectorCount() const { return m_loadedSectorCount; }
    uint32_t loadedEntityEstimate() const { return m_loadedEntityEstimate; }

  private:
    Sector& getOrCreateSector(const SectorCoord& coord);
    void generateSectorSpawns(Sector& sector);
    void markLoaded(Sector& sector);
  ...
  private:
    ...
    uint32_t m_loadedSectorCount = 0;
    uint32_t m_loadedEntityEstimate = 0;
  };
  ...
  {
    uint32_t maxActiveSectors = 25u;
    uint32_t activeRadiusSectors = 2u;
    uint32_t maxEntitiesBudget = 4096u;
